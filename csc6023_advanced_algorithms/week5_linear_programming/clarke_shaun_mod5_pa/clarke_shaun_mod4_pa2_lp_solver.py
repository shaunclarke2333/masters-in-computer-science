...
class LinearProgSolver:
    ...
    def summary(self) -> None:
        ...
        self.best_outcome = round(best_val, 6)

        print("\nBest Option")
        print(f"{best_label} is best: x = {best_x},  f(x) = {self.best_outcome}")


    # Print tbale
    def print_setup_chart(self) -> None:
        """
        This jenky method prints a simple, table like slide 16 for the chemical facotry.
        """
        n: int = len(self.c)

        print("\nIf you tilt your head to the left it kinda looks like a table")

        # variable names as column headers across the top
        header: str = "        " 
        i: int = 0
        while i < n:
            header += f"x{i+1:>8}"
        ...
        j: int = 0
        while j < n:
            row_str: str = f"Row {j+1}:"
            i = 0
            while i < n:
                row_str += f"{self.A[j, i]:>9.2f}"
                i += 1
            row_str += f"   |   ≤ {self.b[j]:.2f}"
            print(row_str)
            j += 1
...
def main() -> None:
    ...
    solver.summary()
    solver.print_setup_chart()

    # print_setup_chart(c, A, b)
...
# -- single (one variable at a time) --
# Only var x1: x = [500.   0.],  f(x) = 25000.0
# Only var x2: x = [  0. 500.],  f(x) = 20000.0

# Balanced Options:
# Balanced x = [375. 250.],  f(x) = 28750.0

# Best Option
# Balanced is best: x = [375. 250.],  f(x) = 28750.0

# If you tilt your head to the left it kinda looks like a table
#         x       1x       2   |   Limit
# --------------------------------
# Row 1:     2.00     3.00   |   ≤ 1500.00
# Row 2:     2.00     1.00   |   ≤ 1000.00
# Profit:    50.00    40.00   |
